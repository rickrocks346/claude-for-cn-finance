...
- **3 个校验脚本**（check.py、validate.py、sync-agent-skills.py）
...
├── managed-agent-cookbooks/             ← Managed Agent cookbooks（Phase 9+）
├── scripts/
...
2. 运行 `python3 scripts/check.py` 校验所有 manifest（当前: 35 checks）
3. 运行 `python3 scripts/validate.py` 验证 SKILL.md 格式（当前: 24 files）
